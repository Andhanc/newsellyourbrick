...
function DebtsDesktopFilters({
  ...
  const activeChips = useMemo(() => {
    ...
    if (riskFilter !== 'all') {
      const item = RISK_TYPE_ITEMS.find((i) => i.value === riskFilter)
      if (item) {
        chips.push({
          id: `risk-${riskFilter}`,
          label: t(item.labelKey),
  ...
  return (
    <aside className="auction-desktop-filters" aria-label={t('filters')}>
      ...
      <div className="auction-desktop-filters__sections">
        ...
        <FilterSection
          title={t('debtsFilterRisk')}
          open={openSections.risk}
          onToggle={() => toggleSection('risk')}
        >
          <ul className="auction-desktop-filters__checklist">
            {RISK_TYPE_ITEMS.map((item) => (
              <li key={item.value}>
                <label className="auction-desktop-filters__check">
                  <input
                    type="checkbox"
                    checked={riskFilter === item.value}
                    onChange={() => setRiskFilter(riskFilter === item.value ? 'all' : item.value)}
                  />
                  <span className="auction-desktop-filters__check-box" aria-hidden />
                  <span>{t(item.labelKey)}</span>
                </label>
