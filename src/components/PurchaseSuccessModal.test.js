...
test('purchase confirmation distinguishes shares from a reservation', () => {
  ...
  assert.match(context, /snapshot\.purchaseKind === 'share'/)
  assert.match(context, /navigate\(`\/profile\/purchased\/\$\{snapshot\.id\}`/)
  assert.match(context, /navigate\('\/profile', \{ replace: true \}\)/)
  assert.match(context, /document\.body\.style\.overflow = ''/)
  assert.match(context, /import\.meta\.env\.DEV/)
  assert.match(context, /buyer_success_preview/)
  assert.match(bridge, /openedSessionRef/)
  assert.match(bridge, /openPurchaseSuccess\(\{ \.\.\.result\.snapshot, purchaseKind: kind \}\)/)
})
