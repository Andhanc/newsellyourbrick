...
test('turns only the phone layout into a bounded scroll-snap rail', () => {
  ...
  assert.match(mobile, /\.hr-strategy-stat-card__visual\s*\{[\s\S]*?inset:\s*0/)
  assert.match(mobile, /\.hr-strategy-stat-card__visual\s*\{[\s\S]*?width:\s*100%[\s\S]*?height:\s*100%/)
  assert.match(mobile, /\.hr-strategy-stat-card__visual\s*\{[\s\S]*?object-fit:\s*cover[\s\S]*?opacity:\s*0\.72/)
  assert.match(mobile, /\.hr-strategy-stat-card__content\s*\{[\s\S]*?max-width:\s*75%/)
  assert.match(mobile, /\.hr-strategy-stat-card \.hr-strategy-stat-card__headline\s*\{[\s\S]*?font-size:\s*clamp\(2rem,\s*9vw,\s*2\.65rem\)/)
  assert.match(mobile, /\.hr-strategy-stat-card \.hr-strategy-stat-card__text\s*\{[\s\S]*?max-width:\s*20ch/)
